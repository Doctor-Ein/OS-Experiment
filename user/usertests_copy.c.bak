...
forkforkfork(char *s)
{
  ...
  sleep(50); // add 30 ticks for slower nested virtualization environments
  close(open("stopforking", O_CREATE|O_RDWR));
  wait(0);
  sleep(10); // one second
}
...
drivetests(int quick, int continuous, char *justone) {
  do {
    ...
    // Single-test mode is used by trace children; skip global memory sweeps
    // so the target test starts promptly on slow virtualized environments.
    if(justone == 0)
      free0 = countfree();
    ...
    if(justone == 0 && (free1 = countfree()) < free0) {
